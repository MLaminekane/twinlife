...
export function applyDirective(directive, context, get) {
    ...
    if (directive.buildingRemove) {
        directive.buildingRemove.forEach(idOrName => {
            const index = buildings.findIndex(b => b.id === idOrName ||
    ...
    if (directive.peopleRemove) {
        directive.peopleRemove.forEach(remove => {
            if (remove.all) {
                ...
                const index = people.findIndex(p => p.name.toLowerCase().includes(remove.name.toLowerCase()));
                if (index !== -1) {
    ...
    if (directive.settings) {
        if (typeof directive.settings.glow === 'boolean')
            settings.glow = directive.settings.glow;
        if (typeof directive.settings.shadows === 'boolean')
            settings.shadows = directive.settings.shadows;
        if (typeof directive.settings.labels === 'boolean')
            settings.labels = directive.settings.labels;
    }
    // Environment
