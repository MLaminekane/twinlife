...
export const useStore = create((set, get) => ({
    ...
    applyDirective: (d) => set(state => {
        ...
        if (result.buildings && result.settings?.visibleBuildings) {
            ...
            const updatedVisible = new Set([...result.settings.visibleBuildings].filter(id => allBuildingIds.has(id)));
    ...
    tick: (dt) => set(state => {
        ...
        processDepartmentDynamics(dt, state.departments, state.buildings, state.scenario, state.deptFlashes, state.deptInteractions, state.news);
        ...
        for (let i = 0; i < state.people.length; i++) {
            ...
            if (shouldCheckSocial && p.state && p.state.currentActivity !== 'sleep' && p.state.currentActivity !== 'work') {
                ...
                for (let j = i + 1; j < Math.min(i + 10, state.people.length); j++) {
                    ...
                    if (!other || !other.state)
                        continue;
                    if (other.state.currentActivity === 'talking' || other.state.currentActivity === 'sleep')
                        continue;
            ...
            if (decision.targetId)
                p.targetBuildingId = decision.targetId;
            if (decision.mood)
                p.state.mood = decision.mood;
        }
    ...
    applyAgentActions: (acts) => set(state => {
        ...
        applyAgentActionsLogic(acts, state.agents, state.departments, state.buildings, state.deptFlashes, state.deptInteractions, newNews, state.scenario, state.people);
