import type { Agent, AgentAction, AgentRole, Department, Building, NewsItem } from './types'
...
export function applyAgentActionsLogic(
  acts: AgentAction[],
  agents: Agent[],
  ...
  news: NewsItem[],
  scenario: { investmentAI: number; investmentHumanities: number },
  people: any[]
) {
    ...
    if (a.challenge) {
      ...
      if (from && to && from !== to) {
        ...
        if (bFrom) {
          ...
          if (bb) bb.activity = Math.min(1, bb.activity + 0.02)
        }
        if (bTo) {
          const bb = buildings.find(x => x.id === bTo.buildingId)
          if (bb) bb.activity = Math.max(0, bb.activity - 0.03)
        }
