import { Person, Building, Environment } from '../state/types'
    ...
    if (person.state.currentActivity !== scheduledTask.activity) {
        ...
        if (isLate || Math.random() < 0.01) {
            ...
            if (targetId) {
                ...
                if (person.state.history.length > 5) person.state.history.shift()
                return { targetId }
            }
        }
    }
...
function chooseLeisureLocation(p: Person, buildings: Building[], env: Environment): string {
    // Filter candidates
    ...
    // Logic
    if (p.traits.energy < 0.3) return p.schedule.find(s => s.activity === 'sleep')?.targetId || buildings[0].id // Go home if tired
...
function chooseFoodLocation(p: Person, buildings: Building[]): string {
