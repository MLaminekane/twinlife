...
export function PersistGate() {
  ...
  useEffect(() => {
    ...
    const unsub1 = useStore.subscribe((s) => {
      try { localStorage.setItem(S_KEY, JSON.stringify(s.scenario)) } catch {}
    })
    const unsub2 = useStore.subscribe((s) => {
      try { localStorage.setItem(SET_KEY, JSON.stringify({ running: s.settings.running, speed: s.settings.speed, glow: s.settings.glow, shadows: s.settings.shadows, labels: s.settings.labels })) } catch {}
    })
